...
import time
import dataprocessing_bpp5 as boxes
import threading
...
def addBox():
    ...
    while True:
        ...
        # if toColorHandle is not None:
        #     simRet, toColorPosition = vrep.simxGetObjectPosition(robot.clientID, toColorHandle, -1, vrep.simx_opmode_blocking)
        #     if robot.previewSensorPosition[0] - toColorPosition[0] > 0.05 and np.sum(toColorPosition)!=0:
        #         robot.setColor(toColorHandle,colors.hex2color(colors.cnames['palegreen']))
        #         toColorIndex += 1
        #
        # if toColorIndex < len(toColorList):
        #     toColorHandle = toColorList[toColorIndex]
        # else:
        #     toColorHandle = None

        if (toProduce == 0 or (abs(lastPosition[0] - initX) > 0.3 and abs(lastPosition[0] - initX) <1)) and toProduce < boxesNum:
        ...
        if toProduce >= len(boxes) and toColorIndex >= len(boxes):
            break
